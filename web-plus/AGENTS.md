...
- `web-plus-license`：与用户登录 Token 隔离的签名许可证内核，不依赖 Spring 或制品构建工具。
- `web-plus-license-starter`：显式引入后强制授权，支持在线短期租约和离线签名许可证；不聚合进通用 starter，不提供运行时关闭开关。
...
- `web-plus-starter`：聚合通用能力，强制授权 starter 由业务项目独立选择。
