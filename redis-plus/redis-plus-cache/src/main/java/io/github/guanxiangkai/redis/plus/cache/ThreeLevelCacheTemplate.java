...
import io.github.guanxiangkai.redis.plus.core.exception.RedisCacheException;
 ...
 *
 * // 注解式（由 {@link io.github.guanxiangkai.redis.plus.cache.aop.ThreeLevelCacheAspect} 驱动）
...
public class ThreeLevelCacheTemplate {
    ...
    private final CacheRuntimePolicy runtimePolicy;
    /**
     ...
     * 创建三级缓存模板。
     *
     * @param runtimePolicy 回源、清理和批量缓存回填共享的运行策略
     */
    public ThreeLevelCacheTemplate(LocalCacheProvider l1,
        ...
        this.runtimePolicy = java.util.Objects.requireNonNull(runtimePolicy, "runtimePolicy 不能为空");
    }
    ...
    public void clear(String cacheName) {
        observeCacheVoid(RedisPlusObservationType.CACHE_EVICT, cacheName, "*", () -> {
            ...
            ScanOptions options = ScanOptions.scanOptions().match(pattern)
                    .count(runtimePolicy.clearScanBatchSize()).build();
            List<String> batch = new ArrayList<>(runtimePolicy.clearDeleteBatchSize());
            try (Cursor<String> cursor = redisTemplate.scan(options)) {
                while (cursor.hasNext()) {
                    batch.add(cursor.next());
                    if (batch.size() >= runtimePolicy.clearDeleteBatchSize()) {
    ...
    private <T> T loadFromL3(String cacheName, String key, String fullKey,
        ...
        CacheLoadProtection.LockHandle handle =
                loadProtection.acquire(lockKey,
                        runtimePolicy.loadLockWait().toMillis(),
                        runtimePolicy.loadLockLease().toMillis(),
        ...
        try (handle) {
            ...
            if (loaded == null) {
                Duration nullTtl = runtimePolicy.resolveNullValueTtl(ttl);
    ...
    private <T> T observeCache(RedisPlusObservationType type,
        ...
        try {
            return observer.observe(type,
                    ObservationTags.cacheName(cacheName),
                    ObservationTags.cacheKey(key),
                    supplier);
        } catch (Throwable t) {
