...
import io.github.guanxiangkai.redis.plus.queue.AckQueue;
 ...
 *   <li>建议配合 {@link io.github.guanxiangkai.redis.plus.queue.spi.QueueRetryStrategy}
 *       和 {@link io.github.guanxiangkai.redis.plus.queue.spi.DeadLetterHandler} 使用</li>
 * </ul>
 *
 * @param <T> 消息类型
 * @author guanxiangkai
...
public class RedisStreamQueue<T> implements AckQueue<T> {
    ...
    private static final String CONDITIONAL_TRIM_SCRIPT = """
    ...
    public RedisStreamQueue(String queueName,
        ...
        this.consumerName = "consumer-" + IdUtil.fastSimpleUUID().substring(0, 8);
        ...
        this.batchSize = runtimePolicy.batchSize();
        this.observer = runtimePolicy.observer();
        this.reclaimOnStart = runtimePolicy.reclaimOnStart();
        this.pendingReclaimIdleTime = runtimePolicy.pendingReclaimIdleTime();
        this.maxStreamLength = runtimePolicy.maxStreamLength();
    ...
    /** 隔离处理完成后尝试确认消息；确认失败时保留在 PEL 中。 */
    private void ackQuietly(RecordId id) {
        try {
            acknowledge(id);
        } catch (Exception e) {
            log.warn("[redis-plus] 隔离消息 ACK 失败，消息留在 PEL，id={}, stream={}", id, streamKey, e);
        }
    }
    ...
    private void trimStreamSafely() {
        Long trimmed = scriptExecutor.execute(CONDITIONAL_TRIM_SCRIPT, Long.class,
    ...
    void processRecord(MapRecord<String, Object, Object> record, Consumer<T> consumer) {
        try {
            ...
            log.error("[redis-plus] Stream 消息处理异常，id={}, stream={}",
    ...
    private QueueDelivery<T> toDelivery(MapRecord<String, Object, Object> record) {
        ...
        if (payload == null) {
            handlePoisonMessage(record, null, QueuePoisonReason.MISSING_PAYLOAD,
                    new IllegalArgumentException("Stream 消息缺少 payload 字段"));
            return null;
        }
        String rawPayload = payload.toString();
        try {
            T message = serializer.deserialize(rawPayload, messageType);
            if (message == null) {
                throw new IllegalStateException("反序列化结果不能为空");
            }
            return new StreamQueueDelivery(record.getId(), message);
        } catch (Exception e) {
            handlePoisonMessage(record, rawPayload, QueuePoisonReason.DESERIALIZATION_FAILED, e);
            return null;
        }
    }

    private void handlePoisonMessage(
            MapRecord<String, Object, Object> record,
            String rawPayload,
            QueuePoisonReason reason,
            Exception cause
    ) {
        QueuePoisonMessage poisonMessage = new QueuePoisonMessage(
                QueueStorageType.STREAM,
                queueName,
                ...
                cause);
        try {
            poisonMessageHandler.handle(poisonMessage);
            ackQuietly(record.getId());
        } catch (Exception handlingFailure) {
            log.error("[redis-plus] 毒消息隔离失败，消息保留在 PEL，queue={}, id={}, reason={}",
                    queueName, record.getId(), reason, handlingFailure);
        }
    }

    @SuppressWarnings("unchecked")
    private StreamOffset<String>[] streamOffset(ReadOffset offset) {
        return new StreamOffset[]{StreamOffset.create(streamKey, offset)};
    }
