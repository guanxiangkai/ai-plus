package io.github.guanxiangkai.redis.plus.ratelimit.annotation;
 ...
 * 超出限流阈值时抛出 {@link io.github.guanxiangkai.redis.plus.core.exception.RedisPlusException}。
 *
 * <p>示例：
 * <pre>
 * // 每个用户每分钟最多 100 次
 ...
 * {@literal @}RateLimit(key = "'api:order:create'", limit = 500, window = 1, algorithm = "SLIDING_WINDOW")
 * public Order createOrder(OrderRequest req) { ... }
 *
 * // 令牌桶：容量 200，每秒补充 50 个令牌
 * {@literal @}RateLimit(key = "'api:order:create'", algorithm = "TOKEN_BUCKET",
...
public @interface RateLimit {
    ...
    /**
     * 限流算法名称。内置值：FIXED_WINDOW、SLIDING_WINDOW、TOKEN_BUCKET、LEAKY_BUCKET。
     * 用户可注册同名 {@code RateLimitAlgorithm} 扩展自定义算法。
     */
