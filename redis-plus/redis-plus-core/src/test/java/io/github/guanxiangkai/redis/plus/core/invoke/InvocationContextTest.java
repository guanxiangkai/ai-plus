...
class InvocationContextTest {
    ...
    @Test
    void spelSupport_cachesParsedExpression() throws Exception {
        Method method = TestService.class.getMethod("find", String.class);
        InvocationContext ctx = InvocationContext.of(new TestService(), method, new Object[]{"42"});
        int before = InvocationContextSpelSupport.cachedExpressionCount();

        assertEquals("user:42", InvocationContextSpelSupport.resolveToString(ctx, "'user:' + #id"));
        assertEquals("user:42", InvocationContextSpelSupport.resolveToString(ctx, "'user:' + #id"));

        assertEquals(before + 1, InvocationContextSpelSupport.cachedExpressionCount());
    }

    @Test
    @SuppressWarnings("NullAway")
    void spelSupport_nullExpression_resolvesToEmptyString() throws Exception {
        Method method = TestService.class.getMethod("find", String.class);
        InvocationContext ctx = InvocationContext.of(new TestService(), method, new Object[]{"42"});

        assertEquals("", InvocationContextSpelSupport.resolveToString(ctx, null));
    }

    @Test
    @SuppressWarnings("NullAway")
    void spelSupport_nullEvaluationResult_resolvesToEmptyString() throws Exception {
        Method method = TestService.class.getMethod("find", String.class);
        InvocationContext ctx = InvocationContext.of(new TestService(), method, new Object[]{null});

        assertEquals("", InvocationContextSpelSupport.resolveToString(ctx, "#id"));
    }

    @Test
    @SuppressWarnings("NullAway")
    void resolveKey_nullCustomResolverResult_resolvesToEmptyString() throws Exception {
        Method method = TestService.class.getMethod("find", String.class);
        InvocationContext ctx = InvocationContext.of(new TestService(), method, new Object[]{"42"});

        assertEquals("", InvocationContexts.resolveKey(ctx, "#id", (_, _) -> null));
    }
